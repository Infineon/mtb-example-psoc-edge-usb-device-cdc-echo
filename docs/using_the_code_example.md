...
The following example clones the "[CDC echo application using emUSB-device](https://github.com/Infineon/mtb-example-psoc-edge-usb-device-cdc-echo)" application with the desired name "CdcEcho" configured for the *KIT_PSE84_EVAL_EPC2* BSP into the specified working directory, *C:/mtb_projects*:

   ```
   project-creator-cli --board-id KIT_PSE84_EVAL_EPC2 --app-id mtb-example-psoc-edge-usb-device-cdc-echo --user-app-name CdcEcho --target-dir "C:/mtb_projects"
   ```
